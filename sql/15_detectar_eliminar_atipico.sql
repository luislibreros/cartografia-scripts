...
ALTER TABLE "{{esquema}}".placa_ajustado ADD COLUMN IF NOT EXISTS atipico TEXT;
UPDATE "{{esquema}}".placa_ajustado SET atipico = NULL;

ALTER TABLE "{{esquema}}".placa_ajustado ADD COLUMN IF NOT EXISTS _flag_gen         BOOLEAN DEFAULT FALSE;
ALTER TABLE "{{esquema}}".placa_ajustado ADD COLUMN IF NOT EXISTS _flag_cluster     BOOLEAN DEFAULT FALSE;
ALTER TABLE "{{esquema}}".placa_ajustado ADD COLUMN IF NOT EXISTS _flag_iqr         BOOLEAN DEFAULT FALSE;
ALTER TABLE "{{esquema}}".placa_ajustado ADD COLUMN IF NOT EXISTS _flag_knn         BOOLEAN DEFAULT FALSE;
ALTER TABLE "{{esquema}}".placa_ajustado ADD COLUMN IF NOT EXISTS _flag_contexto    BOOLEAN DEFAULT FALSE;
ALTER TABLE "{{esquema}}".placa_ajustado ADD COLUMN IF NOT EXISTS _protegida        BOOLEAN DEFAULT FALSE;
ALTER TABLE "{{esquema}}".placa_ajustado ADD COLUMN IF NOT EXISTS _score_total      INTEGER DEFAULT 0;
ALTER TABLE "{{esquema}}".placa_ajustado ADD COLUMN IF NOT EXISTS _detalle_atipico  TEXT;

UPDATE "{{esquema}}".placa_ajustado SET
...
CREATE INDEX IF NOT EXISTS idx_placa_mavvial ON "{{esquema}}".placa_ajustado (id_mavvial);
CREATE INDEX IF NOT EXISTS idx_placa_nomvia  ON "{{esquema}}".placa_ajustado (nomvia);
CREATE INDEX IF NOT EXISTS idx_placa_id_capa ON "{{esquema}}".placa_ajustado (id_capa);
...
UPDATE "{{esquema}}".placa_ajustado p SET _flag_gen = TRUE FROM _tmp_base b WHERE p.id_capa = b.id AND b.id_mavvial IN (SELECT id_mavvial FROM _tmp_winner) AND NOT EXISTS (SELECT 1 FROM _tmp_chain ch WHERE ch.id_mavvial = b.id_mavvial AND ch.gen = b.gen);
UPDATE "{{esquema}}".placa_ajustado p SET _protegida = TRUE FROM _tmp_base b WHERE p.id_capa = b.id AND EXISTS (SELECT 1 FROM _tmp_chain ch WHERE ch.id_mavvial = b.id_mavvial AND ch.gen = b.gen);
UPDATE "{{esquema}}".placa_ajustado SET _protegida = TRUE WHERE id_mavvial IN (SELECT id_mavvial FROM _tmp_stats WHERE n < 3);
...
UPDATE "{{esquema}}".placa_ajustado p SET _flag_cluster = TRUE FROM _tmp_clusters c JOIN _tmp_largest l ON c.id_mavvial = l.id_mavvial WHERE p.id_capa = c.id AND c.cluster_id <> l.largest_cluster AND c.id_mavvial IN (SELECT id_mavvial FROM _tmp_csizes GROUP BY id_mavvial HAVING COUNT(*) > 1);
...
UPDATE "{{esquema}}".placa_ajustado p SET _flag_iqr = TRUE FROM _tmp_base b JOIN _tmp_stats s ON b.id_mavvial = s.id_mavvial WHERE p.id_capa = b.id AND s.n >= 5 AND s.iqr > 0 AND (b.val < s.q1 - 1.5 * s.iqr OR b.val > s.q3 + 1.5 * s.iqr);
...
UPDATE "{{esquema}}".placa_ajustado p
SET _flag_knn = TRUE
FROM _tmp_knn k
...
UPDATE "{{esquema}}".placa_ajustado SET _score_total = (_flag_gen::int + _flag_cluster::int + _flag_iqr::int + _flag_knn::int + _flag_contexto::int) WHERE atipico IS NULL;
UPDATE "{{esquema}}".placa_ajustado SET _detalle_atipico = CONCAT_WS(', ', CASE WHEN _flag_gen THEN 'GEN' END, CASE WHEN _flag_cluster THEN 'CLUSTER' END, CASE WHEN _flag_iqr THEN 'IQR' END, CASE WHEN _flag_knn THEN 'KNN' END, CASE WHEN _flag_contexto THEN 'CONTEXTO' END, CASE WHEN _protegida THEN '(PROTEGIDA)' END) WHERE atipico IS NULL AND _score_total > 0;
UPDATE "{{esquema}}".placa_ajustado SET atipico = 'ATIPICO' WHERE _flag_gen = TRUE;
UPDATE "{{esquema}}".placa_ajustado SET atipico = 'ATIPICO' WHERE atipico IS NULL AND _protegida = FALSE AND (_flag_cluster::int + _flag_iqr::int + _flag_knn::int + _flag_contexto::int) >= 2;
UPDATE "{{esquema}}".placa_ajustado SET atipico = 'ATIPICO' WHERE atipico IS NULL AND _protegida = FALSE AND _flag_contexto = TRUE AND id_mavvial IN (SELECT id_mavvial FROM _tmp_stats WHERE n <= 4);
