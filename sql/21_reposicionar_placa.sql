...
CREATE INDEX IF NOT EXISTS idx_placa_geom
    ON "luis".placa_sant_corr USING GIST (geom);

CREATE INDEX IF NOT EXISTS idx_manzana_geom
    ON "luis".manzana_santiago USING GIST (geom);

CREATE INDEX IF NOT EXISTS idx_mavvial_fin_geom
    ON "luis".mavvial_santiago USING GIST (geom);

CREATE INDEX IF NOT EXISTS idx_mavvial_fin_id_capa
    ON "luis".mavvial_santiago (id_capa);

CREATE INDEX IF NOT EXISTS idx_placa_id_mavvial
    ON "luis".placa_sant_corr (id_mavvial);

CREATE INDEX IF NOT EXISTS idx_placa_porcentaje_cambio
    ON "luis".placa_sant_corr (porcentaje_cambio)
    WHERE porcentaje_cambio > 30;

ANALYZE "luis".placa_sant_corr;
ANALYZE "luis".manzana_santiago;
ANALYZE "luis".mavvial_santiago;
...
BEGIN
    ...
    EXECUTE format(
        'DROP TABLE IF EXISTS "luis".placa_sant_corr_bkp_%s; '
        'CREATE TABLE "luis".placa_sant_corr_bkp_%s AS '
        'SELECT * FROM "luis".placa_sant_corr WHERE porcentaje_cambio > 30;',
        _ts, _ts
    );
    RAISE NOTICE '✔ Respaldo (solo filtrados): "luis".placa_sant_corr_bkp_%', _ts;

    SELECT count(*) INTO _total
      FROM "luis".placa_sant_corr
     WHERE porcentaje_cambio > 30;
    ...
    RAISE NOTICE '  Respaldo              : "luis".placa_sant_corr_bkp_%', _ts;
    RAISE NOTICE '════════════════════════════════════════════════════════════';
...
SELECT count(*) AS puntos_fuera_manzana
 ...
 WHERE p.porcentaje_cambio > 30
   AND NOT EXISTS (
    SELECT 1 FROM "luis".manzana_santiago mz
...
FROM (
    ...
    FROM "luis".placa_sant_corr p
    JOIN "luis".placa_sant_corr_bkp_YYYYMMDD_HHMMSS bk USING (id)
    WHERE p.porcentaje_cambio > 30
) sub;
*/
...
/*
   ...
   BEGIN;
     UPDATE "luis".placa_sant_corr p
        SET geom = bk.geom
       FROM "luis".placa_sant_corr_bkp_YYYYMMDD_HHMMSS bk
      WHERE p.id = bk.id;
   COMMIT;

   -- DROP TABLE "luis".placa_sant_corr_bkp_YYYYMMDD_HHMMSS;
*/
